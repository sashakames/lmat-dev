...
## Stores rank information for each taxonomy node to support dynamic prunning of taxids
## Not used now
##species_map="$LMAT_DIR/species_map.ncbi.segment.dat"
...
for db in $dlst ; do 
      ...
      if [ $db != "$markerdb" ] ; then
         ...
         ##32-bit version
         rprog=${bin_dir}read_label_32
         fstr=""
         #rprog=${bin_dir}read_label_16
         #fstr="-f $tidmap"

         use_min_score=$min_score
         pstr="-g 1000 -m $LMAT_DIR/numeric_ranks"
         #pstr="-g 20000 -m $LMAT_DIR/numeric_ranks"
         #min_kmer_str="-z 20"
      ...
      if [ -e $db ] && [ $do_rl == 1 ] ; then
         if [ ! -e $fastsum_file ] || [ $overwrite == 1 ] ; then
            echo "Process $query_file [$overwrite] [$fastsum_file]"
