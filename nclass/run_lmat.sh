...
for db in $dlst ; do 
      ...
      if [ -e $db ] && [ $do_rl == 1 ] ; then
         if [ ! -e $fastsum_file ] || [ $overwrite == 1 ] ; then
            ...
            cmd="$rprog $min_kmer_str $fstr $pstr -u $taxfile -x $use_min_score -j $min_read_kmer -l $hbias -b $sdiff $vstr $nullmstr -e $depthf -p -t $threads -i $query_file -d $db -c $taxtree -o $rlofile >& $logfile"
	    echo $cmd
	    $cmd
            min_reads=1
