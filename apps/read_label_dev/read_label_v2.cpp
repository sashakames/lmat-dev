...
fill_in_labels(const TaxTree& taxtree, vector<uint32_t>& row, map<uint32_t,uint16_t> & taxids, const hmap_t& tax2idx, const hmap_t& idx2taxid, const set<uint32_t> &taxid_lst) {
  cout << "starting fill_in_labels\n";
  //   bool verbose=true;

  //for (map<uint32_t,uint16_t>::iterator t = taxids.begin(); t != taxids.end(); t++) {
  //for (set<uint32_t>::iterator t = taxid_list.begin(); t != taxid_lst.end(); t++) {
  for(unsigned tax_idx = 0; tax_idx < taxid_lst.size(); ++tax_idx) {
    ...
    if (taxids.find(tax_val) == taxids.end()) {
      cout << "tax_val: " << tax_val << " count: 0\n";
    } else {  
      cout << "tax_val: " << tax_val << " count: " <<  taxids[tax_val] << endl;
    }
    ...
    if ( !row_tax_val_node->isLeaf() ) {
      ...
      if ( has_cnt_sum > 0 ) {
        ...
        cout << "    final count: " << has_cnt_sum << " for tax id: " << tax_val << endl;
        //if (verbose) cout<<"Final Tax Node Val "<<tax_val<<" final transfer cnt for taxid="<<row[tax_idx]<<endl;
      }
    }
  }

  for (map<uint32_t,uint16_t>::const_iterator t = taxids.begin(); t != taxids.end(); t++) {
    assert(tax2idx.find(t->first) != tax2idx.end());
    hmap_t::const_iterator z = tax2idx.find(t->first);
    //const int z = t->second;
    row[z->second] = t->second;
  }
...
cout << ">>>>>>>>>>> calling fill_in for  pos: " << pos << endl;
         ...
         for (tax_data_t::const_iterator t = label_vec[pos].second.begin(); t != label_vec[pos].second.end(); t++) {
           tmp[t->first] = t->second;
         }

         fill_in_labels(tax_tree,label_matrix[pos], tmp, tax2idx, idx2taxid, taxid_lst);
         //fill_in_labels(tax_tree,label_matrix[pos], tmp, idx2taxid);
         //fill_in_labels(tax_tree,label_vec[pos].second,idx2taxid);
         ++cnt_fnd_kmers;
