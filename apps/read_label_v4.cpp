   ...
   while(ifs_lst>>read_len>>file) {
      ...
      while(ifs.getline(buff,buff_size)) {
         ...
         for(unsigned bin=0; (signed)bin < num_bins; ++bin) {
            ...
            // when random model failed to generate a single random matche
...
construct_labels(const TaxTree<TID_T>& tax_tree, const vector<label_info_t>& label_vec, const list<TID_T>& taxid_lst, const hmap_t& tax2idx, const hmap_t& idx2taxid, 
                 ofstream& ofs, size_t mer_len, const ScoreOptions& sopt, int bin_sel, int min_valid_kmers) {
   ...
   assert( mtch != sopt._rand_hits.end() ); 
   ...
   const usmap_t& equiv_class = (*e_mtch).second;
      ...
      for(unsigned tax_idx = 0; tax_idx < num_tax_ids; ++tax_idx) {
         ...
         if( track.find(cval) == track.end() ) {
            ...
            //average
            //track[ cval] += random_prob; //std::min(random_prob,track[cval]);
            //counter [ cval ] += 1;
         }
      ...
      for(unsigned tax_idx = 0; tax_idx < num_tax_ids; ++tax_idx) {
         ...
         const string cval = (*chk).second;
         //try taking average
         //const float random_prob = track[cval] / counter[cval];
         //try taking max
         const float random_prob = track[cval] ;
         ...
         log_sum += log_odds;
         min_val = tax_idx == 0 ? log_odds : min(log_odds,min_val);
         sig_hits++;
...
void proc_line(const TaxTree<TID_T>& tax_tree, int ri_len, string &line, int k_size, INDEXDB<DBTID_T> *table, ofstream &ofs, float threshold, const ScoreOptions& sopt, uint16_t max_count
     ...
     if(ri_len < 0 || ri_len > (signed)line.length()) {
        ...
        if( !taxid_lst.empty() ) {  
           pair< ufpair_t, match_t> mtch = construct_labels(tax_tree,label_vec,taxid_lst,tax2idx,idx2tax,ofs,k_size,sopt,bin_sel,min_kmer); 
...
#pragma omp parallel shared(k_size, query_fn, ofbase, taxtable, tax_tree, sopt,  prn_read,track_matchall,track_nomatchall,track_tscoreall,min_score,min_kmer, in_finished, read_count_in, read_count_out)  private(ifs, finished, pos, ofs, ofname, line, read_buff, hdr_buff, save_hdr)
  {
	...
	while (queue_size < QUEUE_SIZE_MAX && j< 2* n_threads) {
	  ...
	  /*      if ((signed)read_count_in == max_reads){                     
	...
	proc_line(tax_tree, read_buff.length(), read_buff, k_size, taxtable, 
		  ...
		  track_tscore,min_score, min_kmer);
