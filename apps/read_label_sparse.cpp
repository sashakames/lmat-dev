...
//stops TaxTable::ingest after reading 'stop_early' entries from
//a tax_histo file; usage: -h 100000
...
typedef map<uint32_t, uint16_t> tax_data_t;
...
void
fill_in_labels(const TaxTree& taxtree, tax_data_t & taxids, const hmap_t& idx2taxid, const set<uint32_t> &taxid_lst) {
  //   bool verbose=true;

  for (unsigned tax_idx = 0; tax_idx < taxid_lst.size(); ++tax_idx) {
    ...
    if ( !row_tax_val_node->isLeaf() ) {
      map<uint32_t,uint16_t>::const_iterator it = taxids.begin();
      const map<uint32_t,uint16_t>::const_iterator is = taxids.end();
...
void
construct_labels(const TaxTree& tax_tree, vector<label_info_t>& label_vec, const set<uint32_t>& taxid_lst, const hmap_t& tax2idx, const hmap_t& idx2taxid, ofstream& ofs, size_t mer_len, const ScoreOptions& sopt) {
  //   bool verbose = true;
  ...
  for (unsigned pos = 0; pos < label_vec.size(); ++pos) {
    ...
    if ( any > 0 ) {
      ...
      fill_in_labels(tax_tree, label_vec[pos].second, idx2taxid, taxid_lst);
      ++cnt_fnd_kmers;
  ...
  for (unsigned tax_idx = 0; tax_idx < num_tax_ids; ++tax_idx) {
    ...
    float tot_genome_cnt2 = 0, found_genome_cnt2 = 0;
    ...
    for (unsigned pos = 0; pos < label_vec.size(); ++pos) {
      ...
      if ( any_kmer_match[pos] ) {
        if ( sopt._strict_kmer_match && label_vec[pos].second.find(taxid) == label_vec[pos].second.end()) {
          noMatch=true;
          break;
        }
        ...
        if ( sopt._equal_kmer_vote ) {
          ...
          if (label_vec[pos].second.find(taxid) != label_vec[pos].second.end()) {
            assert(label_vec[pos].second[taxid] > 0);
            ++found_genome_cnt;
          }
          ...
          if (label_vec[pos].second.find(taxid) != label_vec[pos].second.end()) {
            found_genome_cnt += label_vec[pos].second[taxid];
          }
        }
      }
    }
  ...
  const float pcnt_cov = (float)coverage /(float)(label_vec.size()+mer_len);
...
void proc_line(const TaxTree& tax_tree, int ri_len, string &line, int k_size, TaxTable *table, ofstream &ofs, float threshold, const ScoreOptions& sopt) {
  ...
  for (unsigned chk = 0; (chk+ri_len) <= line.length(); chk += (ri_len+1)) {
    ...
    //vector<pair<uint16_t,map<uint32_t, uint16_t> > >
    //label_vec[j]: pair<uint16_t,map<uint32_t, uint16_t> >
    //label_vec[j].second: map<uint32_t, uint16_t>
