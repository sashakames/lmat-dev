...
construct_labels(const TaxTree& tax_tree, const vector<label_info_t>& label_vec, const list<uint32_t>& taxid_lst, const hmap_t& tax2idx, const hmap_t& idx2taxid, ofstream& ofs, size_t mer_len, const ScoreOptions& sopt, const vector<bool> & is_forward) {
   ...
   */
   ...
   void retrieve_kmer_labels(INDEXDB* table, const string& read, const size_t mer_len, vector<label_info_t>& label_vec, list<uint32_t>& taxid_lst, hmap_t& tax2idx, hmap_t& idx2tax, bool revStrnd, vector<bool>& is_forward, const hmap_t& dmap, const TaxTree& tax_tree, int16_t max_count) 
   {
   ...
   void proc_line(const TaxTree& tax_tree, int ri_len, string &line, int k_size, INDEXDB *table, ofstream &ofs, const ScoreOptions& sopt, int16_t max_count) {
...
void usage(char *execname)
{
  ...
  << "  -g <string>  - gid_to_tid_filename     [required]\n"
  ...
  << "\n"
  << "example invocation:\n"
  << execname << " -d marker_lib.18 -o output.marker.k=18 -i my_queries.fa -c ../runtime_inputs/kpath_taxonomy.dat -e ../runtime_inputs/depth_for_kpath_taxonomy.dat -g ../runtime_inputs/gid_to_tid_kpath.txt -t 32\n"
...
int main(int argc, char* argv[]) 
{
   ...
   INDEXDB *taxtable;
     ...
     int j = 0;
     FILE *out = fopen(ofbase.c_str(), "w");
     assert(out);
     uint64_t kmer;
     for (INDEXDB::iterator t = taxtable->begin(); t != taxtable->end(); t++) {
       kmer = t->first;
       assert(fwrite(&kmer,8,1,out) == 1);
       ++j;
       if (j % 1000000 == 0) cout << "processed so far: " << j/1000000<<" M" <<endl;
       //cout << t->first << endl;
