...
metag_dir="$HOME/svn/metag_repo-new2"
...
ofile="/mnt/ramfs/$query_fn.$dbname.$model_size.$cutoff.lo.rl_output" 
cfiles="$tdir/cutoffs.kcnt.50.microbe2-18.flst"
logfile="$ofile.log" 
...
$bin/read_label_lo -n $cfiles -e $depthf -w -p -r -t $threads -i $query_file -d $dbfile -c $tdir/kpath_taxonomy.dat -o $ofile -v $thresh -g $gtmap -a -h $cutoff > $logfile
