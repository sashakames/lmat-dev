...
(our current Full database is XXXGB compared to the previous size of 540GB)
...
database".   
     The web site for LMAT is \texttt{http://computation.llnl.gov/lmat}.  
...
%\setlength{\itemindent{15pt}}
   ...
   \\
     ...
     *** TODO: Get size of 1000 taxid threshold version when done

     Our current ``Full'' database (not yet available) is 400 GB in
     size, smaller than what we used for our July 2013 publication in
     Bioinformatics due to several of the key optimizations within
     this release.  We have two smaller ``marker'' database available for 
     ftp download, the sizes of the two databases listed below are respectively 52GB and 16GB uncompressed.  
...
\begin{verbatim}
   ...
   #  Download the marker libary, set <db-name> to kML.18mer.16bit.db (larger)
   #  or kML.18mer.16bit.reduced (smaller) 
...
Example scoring and classification using the included binary and the Marker DB:
...
%
% to run the marker library first and then full database run

Example Run of LMAT:

Recall the environment variable LMAT_DIR must be set and point to the directory containing the files given in the runtime_inputs directory
%for example in bash:
%export LMAT_DIR=../runtime_inputs
%or csh:
%setenv LMAT_DIR ../runtime_inputs
...
cd bin
./run_lmat.sh --marker_library=<path-to-library/kML.18mer.16bit.db --query_file=../bin/simple_list.fna --odir=../tmp --genedb_file=<path-to-gene-database>/gene.20mer.db

Output files:
simple_list.1000.fna.kML.18mer.16mit.db.lo.rl_output.0.30.fastsummary - Shows the initial LMAT search output, bins reads into rank flexible categories
simple_list.1000.fna.kML.18mer.16mit.db.lo.rl_output.0.30.fastsummary.summ - Reassigns higher order ranked reads to a target organism call
simple_list.1000.fna.kML.18mer.16mit.db.lo.rl_output.ras.flst.gene.20mer.db.rl_output.0.01.20.genesummary - gene content summary
simple_list.1000.fna.kML.18mer.16mit.db.lo.rl_output.ras.flst.gene.20mer.db.rl_output[0-9]*.out - initial individually labeled reads
simple_list.1000.fna.kML.18mer.16mit.db.lo.rl_output[0-9]*out.ras.1 - rebinned reads where higher ranked reads were assigned to target organisms
...
The purpose of having two different types of reference databases (--marker_library and --db_file) is for the conveniance of tuning the cutoff values for the two database types differently
...
The output of the initial binning is in the file \texttt{<prefix>.fastsummary}, the file \texttt{<prefix>.krona}, gives the output as human readable lineages that Krona {http://sourceforge.net/p/krona/home/krona/}
can process. \texttt{<prefix>.krona.html} gives the Krona output (when Krona is installed on the user system).
...
A common post processing step is to retrieve reads for a specific organism or set of taxnomic IDs.
...
example usage:
\begin{verbatim}
build_taxid_lst.sh --file_lst=fastsummary.file_lst --sstr=superkingdom,Viruses --idfile=output_taxid_file.txt
\end{verbatim}
...
Output from the summary step is plced in a file \texttt{<prefix>.summ}
