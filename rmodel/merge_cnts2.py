...
for line in tax_obs_strm :
   ...
   while True :
      #if curr_tid == 549 :
         #print "fnd something?",tid,curr_tid,ranks[curr_tid]
         #verbose=True
      if (ranks[curr_tid] == "species" and human_yes) or \
         ...
         if store_rank_val.has_key(curr_tid)   :
...
for tid in qlst :
   ...
   if isEuk.has_key(tid) and use_rank == "genus" :
   ...
   if save_rit == -1 and save_fit == -1 and not_min :
      print "warning did not get minimum observations"
   out_fh.write(str_out+"\n")
   #save.setdefault(tid,str_out)
   #saveid.append( (tid,rval_kcnt[0]) )
   cnt+=1
